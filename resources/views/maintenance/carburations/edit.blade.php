@extends('layouts.app')
@section('title', "GMAO Trans'urb - Modifier Carburation")
@section('content')
    <header class="bg-green-900 shadow-xl mb-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center gap-4">
            ...
            <span class="text-2xl font-bold tracking-wide text-white">Trans'urb GMAO - Modifier Carburation</span>
        </div>
    </header>
    ...
    <div class="max-w-4xl mx-auto">
        <!-- Bouton retour -->
        <div class="mb-6">
            <a href="{{ route('maintenance.carburations.show', $carburation->id) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors duration-200">
                ...
                <form id="carburationForm" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="vehicule_id" name="vehicule_id" required 
                                ...
                                @foreach($vehicules as $vehicule)
                                    <option value="{{ $vehicule->id }}" {{ $carburation->vehicule_id == $vehicule->id ? 'selected' : '' }}>
                                        {{ $vehicule->numero }} - {{ $vehicule->immatriculation }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="chauffeur_id" name="chauffeur_id" required 
                                ...
                                @foreach($chauffeurs as $chauffeur)
                                    <option value="{{ $chauffeur->id }}" {{ $carburation->chauffeur_id == $chauffeur->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="date" id="date_carburation" name="date_carburation" required 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                   value="{{ $carburation->date_carburation->format('Y-m-d') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="time" id="heure_carburation" name="heure_carburation" required 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                   value="{{ $carburation->heure_carburation->format('H:i') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="quantite_litres" name="quantite_litres" step="0.01" min="0.01" required 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                   placeholder="50.00" value="{{ $carburation->quantite_litres }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="prix_litre" name="prix_litre" step="0.01" min="0.01" required 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                   placeholder="650.00" value="{{ $carburation->prix_litre }}">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="type_carburation" name="type_carburation" required 
                                ...
                                <option value="Diesel" {{ $carburation->type_carburation == 'Diesel' ? 'selected' : '' }}>Diesel</option>
                                <option value="Essence" {{ $carburation->type_carburation == 'Essence' ? 'selected' : '' }}>Essence</option>
                                <option value="GPL" {{ $carburation->type_carburation == 'GPL' ? 'selected' : '' }}>GPL</option>
                                <option value="Électrique" {{ $carburation->type_carburation == 'Électrique' ? 'selected' : '' }}>Électrique</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="etat" name="etat" required 
                                ...
                                <option value="Planifiée" {{ $carburation->etat == 'Planifiée' ? 'selected' : '' }}>Planifiée</option>
                                <option value="Effectuée" {{ $carburation->etat == 'Effectuée' ? 'selected' : '' }}>Effectuée</option>
                                <option value="Annulée" {{ $carburation->etat == 'Annulée' ? 'selected' : '' }}>Annulée</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                  placeholder="Informations supplémentaires...">{{ $carburation->notes }}</textarea>
                    </div>
                    
                    <div class="flex justify-end space-x-3 pt-6">
                        <a href="{{ route('maintenance.carburations.show', $carburation->id) }}" 
    ...
    <script>
        ...
        document.getElementById('carburationForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route("maintenance.carburations.update", $carburation->id) }}', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setTimeout(() => {
                        closeSuccessModal();
                        window.location.href = '{{ route("maintenance.carburations.show", $carburation->id) }}';
                    }, 2000);
                } else {
                    ...
                    if (data.errors) {
                        ...
                        showErrorModal('Erreur', data.message || 'Erreur lors de la mise à jour de la carburation');
                    }
                }
            })
            .catch(error => {
                console.error('Erreur:', error);
                showErrorModal('Erreur', 'Erreur lors de la mise à jour de la carburation');
            });
        });
    </script>
@endsection
