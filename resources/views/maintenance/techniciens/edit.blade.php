        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h1 class="text-2xl font-bold tracking-wide text-white">Modifier le technicien</h1>
                        <p class="text-green-100">{{ $technicien->nom_complet }}</p>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="px-6 py-4 bg-gradient-to-r from-green-600 to-green-700">
                <h2 class="text-xl font-semibold text-white">
                    <i class="fas fa-edit mr-2"></i>Modifier le technicien
                </h2>
            </div>
            
            <form id="technicienForm" action="{{ route('maintenance.techniciens.update', $technicien->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="fonction_technique" name="fonction_technique" required 
                               value="{{ $technicien->fonction_technique }}"
                               class="w-full px-3 py-2 border border-green-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    ...
                    <div>
                        ...
                        <select id="niveau_competence" name="niveau_competence" required 
                            ...
                            <option value="Débutant" {{ $technicien->niveau_competence == 'Débutant' ? 'selected' : '' }}>Débutant</option>
                            <option value="Intermédiaire" {{ $technicien->niveau_competence == 'Intermédiaire' ? 'selected' : '' }}>Intermédiaire</option>
                            <option value="Expert" {{ $technicien->niveau_competence == 'Expert' ? 'selected' : '' }}>Expert</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="date_embauche" name="date_embauche" required 
                               value="{{ $technicien->date_embauche ? $technicien->date_embauche->format('Y-m-d') : '' }}"
                               class="w-full px-3 py-2 border border-green-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="statut" name="statut" required 
                            ...
                            <option value="Actif" {{ $technicien->statut == 'Actif' ? 'selected' : '' }}>Actif</option>
                            <option value="Inactif" {{ $technicien->statut == 'Inactif' ? 'selected' : '' }}>Inactif</option>
                            <option value="En Formation" {{ $technicien->statut == 'En Formation' ? 'selected' : '' }}>En Formation</option>
                            <option value="En Congé" {{ $technicien->statut == 'En Congé' ? 'selected' : '' }}>En Congé</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="competences" name="competences" rows="3" 
                              class="w-full px-3 py-2 border border-green-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                              placeholder="Compétences techniques...">{{ $technicien->competences }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="formations_suivies" name="formations_suivies" rows="3" 
                              class="w-full px-3 py-2 border border-green-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                              placeholder="Formations et certifications...">{{ $technicien->formations_suivies }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" 
                              class="w-full px-3 py-2 border border-green-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                              placeholder="Notes et observations...">{{ $technicien->notes }}</textarea>
                </div>
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.errors) {
                    ...
                    showErrorModal('Erreur', data.message || 'Erreur lors de la mise à jour du technicien');
                }
            }
        })
        .catch(error => {
            console.error('Erreur:', error);
            showErrorModal('Erreur', 'Erreur lors de la mise à jour du technicien');
        });
    });
</script>
@endsection
